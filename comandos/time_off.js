...
;(timeout = 10000),
	(exports.run = async (a, b, c, d) => {
		...
		let l = a.users.filter((m) => m.presence.status === 'offline')
		...
		b.channel.send(p).then((q) => q.delete(50000)),
			await l.array().forEach(async (r) => {
				sleep(timeout),
					await r
						.send(j)
						.then(() => console.log(('    [TIME-OFF] Mensagem enviada para ' + r.tag.yellow).green))
						.catch(() => console.log(('    [TIME-OFF] Erro ao enviar mensagem para ' + r.tag).red)),
